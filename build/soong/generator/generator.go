...
// Copyright (C) 2018 The requiemOS Project
//
...
func init() {
	android.RegisterModuleType("requiem_generator", GeneratorFactory)
...
func (g *Module) GenerateAndroidBuildActions(ctx android.ModuleContext) {
	...
	if depRoot == "" {
		...
		depRoot = requiemExpandVariables(ctx, depRoot)
	}

	// Glob dep_files property
	for _, dep_file := range g.properties.Dep_files {
		dep_file = requiemExpandVariables(ctx, dep_file)
	...
	cmd := requiemExpandVariables(ctx, String(g.properties.Cmd))
